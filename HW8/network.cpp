...
{
    int postCounts = 0;
    // traverse through the vector to find how many posts there are
    for (int i = 0; i < posts_.size(); i++)
    {
        postCounts += posts_[i].size();
    }
    // sets the messageid to the postCounts
    post->setMessageId(postCounts);
...
std::string Network::postDisplayString(Post *post)
{
    ...
    if (id < 0 || id >= users_.size())
    {
        return "No id wrote: " + post->toString();
    }
    // store the users id
    User *id2 = users_[id];
    // get the authors name
    std::string author = id2->getName();
...
{
    // Create a new vector to store the amount posts for that person
    std::vector<Post *> posts = getPosts(profileId);

    int chosen;
    // need to check if the user has enough posts
    if (howMany <= posts.size())
    {
        chosen = howMany;
    }
    else
    ...
    // check to make sure that chosen and posts aren't empty
    if (chosen == 0 || posts.empty())
    {
        return "";
    }

    // adds the post to the final string
    std::string final = "";
    bool firstPost = true;
    // have to change to int because size returns size_t
    for (int i = (int)posts.size() - 1; i >= (int)posts.size() - chosen; i--)
    {
        std::string postWords = postDisplayString(posts[i]);
        // checks to see if it is the first post
        if (!firstPost)
        {
            final += "\n\n";
        }
        // adds all of the words in the post to the final string
        final += postWords;
        firstPost = false;
    }
...
int Network::readPosts(char *fname)
{
    ...
    // loop through the users posts

    for (int i = 0; i < numPosts; i++)
    {
        // have to get every single data point of the user
        int messageId;
        in >> messageId;
        // this ignores the new line, so that it doesnt get read
        in.ignore();

        std::string message;
        getline(in, message);
        // you have to do this check to see if there is a tab in the message
        ...
        std::string url;
        getline(in, url);
        // do the same check if the url is empty
        if (!url.empty() && url[0] == '\t')
        {
            url.erase(0, 1);
        }

        // The profileId values might be larger than the current size of posts_ so we have to have this check
        if (posts_.size() <= profileId)
        {
        ...
        if (url.empty())
        {
            // if it is empty then it is a normal post
            Post *p = new Post(profileId, authorId, message, likes);
            p->setMessageId(messageId); // this overwrites the messageId
            posts_[profileId].push_back(p);
        }
        else
        {
            // if it is not empty then it is a linkpost
            Post *p = new LinkPost(profileId, authorId, message, likes, url);
...
int Network::writePosts(char *fname)
{
    ...
    // collect all of the posts into one file
    std::vector<Post *> Posts;
    // gets the post for each user
    for (int i = 0; i < posts_.size(); i++)
    {
        // these two for loops gets each post from each user
        for (int j = 0; j < posts_[i].size(); j++)
        {
            Posts.push_back(posts_[i][j]);
        }
    }

    // we do this compare function because we have to sort out all of the posts by their messageId
    // taking the compare function that is at the top of the file
    std::sort(Posts.begin(), Posts.end(), compare);

    // Pnumber of posts
    out << Posts.size() << std::endl;

    for (int i = 0; i < Posts.size(); i++)
    {
        out << Posts[i]->getMessageId() << std::endl;
        out << "\t" << Posts[i]->getMessage() << std::endl;
        out << "\t" << Posts[i]->getProfileId() << std::endl;
        out << "\t" << Posts[i]->getAuthorId() << std::endl;
        out << "\t" << Posts[i]->getLikes() << std::endl;
        out << "\t" << Posts[i]->getURL() << std::endl;
    }
